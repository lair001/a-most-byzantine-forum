...
describe 'Post' do 
  ...
  before do
    ...
    @thread = Thread.create(title: "nothing here")
    @post1 = Post.create(content: "ipsum lorem", user_id: @user.id, thread_id: @thread.id)
    @post2 = Post.create(content: "blah blah", user_id: @user.id, thread_id: @thread.id)
  end
