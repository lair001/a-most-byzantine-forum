...
describe 'forum_posts/new' do
  ...
  it 'renders a create post form with a field for thread title' do
    use_view_to_login_as(@user1)
  	visit "/forum_threads/#{@thread1.slug}/forum_posts/new?cached_post=My+bad,+dude."
    ...
    expect(page).to have_css("input[type='hidden'][name='cached_route'][value='/forum_threads/#{@thread1.slug}/forum_posts/new']")
  end

  it 'shows errors if post creation is unsuccessful' do
    use_view_to_login_as(@user1)
    visit "/forum_threads/#{@thread1.slug}/forum_posts/new"
